 ...
 *  06.00.00   2015-01-20  AWh     ESCAN00080493    Add support for XCP   
 *                                 ESCAN00080567    Support for compression
 *                                 ESCAN00078391    Tester timeout when no response pending from application 
 *                                                    previous to $34 while parsing Sba ticket in bootloader
 **********************************************************************************************************************/
...
#if ( FBLKBDIAG_14230_GM_VERSION != 0x0600u ) || \
...
# error "Fbl_hdr >= 02.00 interface required."
#endif
...
# endif
   ...
   /* Call Header Module Init here to allow for a response pending in case of SBA-ticket parsing */
   FblHdrInit();
...
static void FblEnterRequestDownload( vuint8 formatId, tFblLength uncompressedMemSize )
{
   ...
   /* Gm specifies to use Header information to evaluated data format (e.g. Compression) information */
   /* Only sende NRC if not compression/encryption is supported and DFI is different to 0. */
   if (formatId != 0)
   {
   ...
   /* Indicate new module download header parsing required state */
   FblHdrSetHdrExpectedState(); /* PRQA S 3109 */ /* MD_CBD_14.3 */
...
static void FblDiagRequestDownload( void )
{
   tFblLength uncompressedMemSize;
 ...
 *  \details     Initialize global data used for processing of received data. Call FblHdrTransferDataProcess
 *               to parse header information and effectivly do download.
 *               Call ApplFblTransferData for user checks and FblHdrTransferDataProcess to process service request.
 **********************************************************************************************************************/
static void FblDiagTransferData( void )
{
   ...
   if (  (kDiagSubDownload != DiagBuffer[kDiagFmtSubparam])
         || (DiagDataLength <= kDiagRqlTransferData)
      )
   {
#if defined( FBL_ENABLE_DEBUG_STATUS )
   ...
   transferPayloadSize = DiagDataLength - kDiagRqlTransferData;

   if ((kFblOk != ApplFblTransferData()) || (kFblOk != FblHdrTransferDataProcess(&DiagBuffer[kDiagFmtDataOffset], transferPayloadSize))) /* PRQA S 3415 */ /* MD_FblDiag_3415 */
   {
      if (kDiagErrorNone == DiagGetError())
      {
...
/* module specific MISRA deviations:
   ...
   MD_FblDiag_3415:
     Reason: Both calls have a uniform reaction, the call to FblHdrTransferDataProcess() need not be executed if the
