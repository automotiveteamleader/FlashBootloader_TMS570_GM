...
/***********************************************************************************************************************
 *  --------------------------------------------------------------------------------------------------------------------
 *  REVISION HISTORY (KbFbl_apHW.h, TMS470)
 *  --------------------------------------------------------------------------------------------------------------------
 *  Version    Date        Author  Change Id        Description
 *  --------------------------------------------------------------------------------------------------------------------
 ...
 *  01.04.00   2006-07-26  Ls                       No changes
 *  01.05.00   2007-11-06  Ls                       No changes
 *  01.06.00   2009-02-27  JHg     ESCAN00033355    Added support for TI OMAP
 *  01.07.00   2009-09-22  Ci      ESCAN00036268    No changes
 *                         JHg     ESCAN00037943    No changes
 *  01.08.00   2010-03-26  Pal     ESCAN00041890    Add support for C_PROCESSOR_TMS470_MSF542
 *  01.09.00   2010-03-26  AWh     ESCAN00055193    No changes
 *  01.10.00   2013-07-04  Ach     ESCAN00061452    No changes
...
#define FBLKBAPI_FRAMENV_GM_VERSION           0x0500u
#define FBLKBAPI_FRAMENV_GM_RELEASE_VERSION   0x00u
...
#else
#define ApplFblNVMReadSBATicket(buffer) ApplFblNvReadSBA_ticket(0, (buffer))
#define ApplFblNVMReadECUID(buffer) ApplFblNvReadECU_ID(0, (buffer))
#define ApplFblNVMReadKeyNBID(nbidInfo) ApplFblNvReadKey_NBID(0, (V_MEMRAM1 vuint8 V_MEMRAM2*)&(nbidInfo)->stored)
#define ApplFblNVMWriteKeyNBID(nbidInfo) ApplFblNvWriteKey_NBID(0, (V_MEMRAM1 vuint8 V_MEMRAM2*)&(nbidInfo)->received)
# if defined( FBL_ENABLE_MULTI_PROCESSOR )
  /* In case of Multi processor configuration implement AppNBID handling in function. */
# else
#  define ApplFblNVMReadAppNBID(nbidInfo) ApplFblNvReadApp_NBID(0, (V_MEMRAM1 vuint8 V_MEMRAM2*)&(nbidInfo)->stored)
#  define ApplFblNVMWriteAppNBID(nbidInfo) ApplFblNvWriteApp_NBID(0, (V_MEMRAM1 vuint8 V_MEMRAM2*)&(nbidInfo)->received)
# endif
#endif
...
tFblResult ApplFblNVMReadECUID( V_MEMRAM1 vuint8 V_MEMRAM2 * const buffer );

tFblResult ApplFblNVMReadKeyNBID( V_MEMRAM1 tNBIDInfo V_MEMRAM2 V_MEMRAM3 * const keyNBIDInfo );
tFblResult ApplFblNVMWriteKeyNBID( const V_MEMRAM1 tNBIDInfo V_MEMRAM2 * const keyNBIDInfo );
#endif
...
tFblResult ApplFblNVMWriteAppNBID( const V_MEMRAM1 tNBIDInfo V_MEMRAM2 * const appNBIDInfo );
#endif
...
/***********************************************************************************************************************
 *  END OF FILE: KBFBL_APNVFRAMEOEM.H
 **********************************************************************************************************************/
